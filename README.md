...
```bash
data-ingest$ sam build --use-container
```
...
```bash
data-ingest$ sam logs -n StockCheckerFunction --stack-name data-ingest --tail
```
...
```bash
data-ingest$ pip install pytest pytest-mock --user
data-ingest$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name data-ingest
```
